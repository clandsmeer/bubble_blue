...
class VN100Manager(Node):
    def __init__(self):
        ...
        # Configure the two UART ports (imu port for imu, kf port for kalman filter)
        self.port1 = self.get_parameter('port1').get_parameter_value().string_value
        self.port2 = (
            self.get_parameter('port2').get_parameter_value().string_value
        )
        self.baudrate = (
            self.get_parameter('baudrate').get_parameter_value().integer_value
        )
        self.print_output = (
            self.get_parameter('print_output').get_parameter_value().bool_value
        )
        self.read_time = (
            self.get_parameter('read_time').get_parameter_value().double_value
        )
        self.imu_accel_cov = (
            self.get_parameter('imu_accel_cov').get_parameter_value().double_value
        )
        self.imu_ang_vel_covariance = (
            self.get_parameter('imu_ang_vel_covariance')
            .get_parameter_value().double_value
        )
        self.imu_orientation_cov = (
            self.get_parameter('imu_orientation_cov')
            .get_parameter_value().double_value
        )
        self.init_orient = (
            self.get_parameter('init_kf_orient_cov')
            .get_parameter_value().double_value
        )
        ...
        try:
            self.ser_port1 = serial.Serial(
                self.port1,
                self.baudrate,
                timeout=1,
                rtscts=True
            )
            self.ser_port2 = serial.Serial(
                self.port2,
                self.baudrate,
                timeout=1,
                rtscts=True
        ...
        self.get_logger().info(
            f'The imu port is {self.port1}, and the kf port is {self.port2}'
        )

        # Create the publishers for the ros nodes
        self.imu_pub = self.create_publisher(Imu, '/vectornav/Imu_body', 10)
        self.orient_pub = self.create_publisher(
            PoseWithCovarianceStamped,
            '/vectornav/filtered_orientation', 10
        )
            ...
            try:
                ...
                if self.publishing_data:
                    ...
                    if msg_type == '$VNYBA':
                        ...
                        # to make it higher than KF-output
                        imu_message.orientation_covariance[0] = self.imu_orientation_cov
                        ...
                        imu_message.angular_velocity_covariance[0] = (
                            self.imu_ang_vel_covariance
                        )
                        imu_message.angular_velocity_covariance[4] = (
                            self.imu_ang_vel_covariance
                        )
                        imu_message.angular_velocity_covariance[8] = (
                            self.imu_ang_vel_covariance
                        )

                        # Linear Acceleration and Associated Covariance
                        imu_message.linear_acceleration.x = (
                            float(parsed_msg[4]) - self.bias_body_x
                        )
                        imu_message.linear_acceleration.y = (
                            float(parsed_msg[5]) - self.bias_body_y
                        )
                        imu_message.linear_acceleration.z = (
                            float(parsed_msg[6]) - self.bias_body_z
                        )
                        imu_message.linear_acceleration_covariance[0] = (
                            self.imu_accel_cov
                        )
                        imu_message.linear_acceleration_covariance[4] = (
                            self.imu_accel_cov
                        )
                        imu_message.linear_acceleration_covariance[8] = (
                            self.imu_accel_cov
                        )
                        ...
                        if self.publishing_data:
                            self.get_logger().warn(
                                'Port 2 should be publishing data, '
                                'but output is still default'
                            )

                    else:
                        # for now, there is no handling of other message types,
                        # so just throw an error here:
                    ...
                    if msg_type == '$VNYBA':
                        ...
                        self.get_logger().warn(
                            'BIAS ESTIMATE CALLED BEFORE IMU OUTPUTTING '
            ...
            try:
                ...
                if self.publishing_data:
                    ...
                    if msg_type == '$VNSTV':
                        ...
                        # TODO: Replace these with periodically sampled
                        # covariances from the vn100
                        ...
                        # for pose.pose.position, indicate that they are not
                        # to be included
                        ...
                        self.get_logger().info(
                            f'Orientation covariance is [{self.orient_x_cov}, '
                            f'{self.orient_y_cov}, {self.orient_z_cov}]'
                        )

                    elif msg_type == '$VNRRG' and int(parsed_msg[1]) == 254:
                        # this is the read register response for the covariance,
                        # read it the same as above
                        self.orient_x_cov = float(parsed_msg[2])
                        self.orient_y_cov = float(parsed_msg[3])
                        self.orient_z_cov = float(parsed_msg[4])

                    elif msg_type == '$VNYMR':
                        # This is the default, so don't worry unless the port
                        # expects to be publishing data
                        if self.publishing_data:
                            self.get_logger().warn(
                                'Port 2 should be publishing data, '
                                'but output is still default'
    ...
    def handle_configure(self, request, response):
        # parse the request
        self.get_logger().info(
            f'Sending message {request.msg} to port {request.port}'
        )
        port = request.port
        try:
            if port == 'port1':
        ...
        if self.print_output:
            ...
            self.get_logger().info(f'Reading Logs for {self.read_time} seconds')
            timeout = self.read_time + time.time()
            try:
                while time.time() < timeout:
                    if port == 'imu':
                        line = self.ser_port1.readline().decode(
                            'ascii',
                            errors='ignore'
                        ).strip()
                        self.get_logger().info(f'FROM IMU PORT: {line}')
                    if port == 'kf':
                        line = self.ser_port2.readline().decode(
                            'ascii',
                            errors='ignore'
                        ).strip()
        ...
        with self.thread_lock:
            if not self.ports_active:
                ...
                if self.port1_thread is None or not self.port1_thread.is_alive():
                    self.port1_thread = threading.Thread(
                        target=self.port1_reader,
                        daemon=True
                    )
                ...
                if self.port2_thread is None or not self.port2_thread.is_alive():
                    self.port2_thread = threading.Thread(
                        target=self.port2_reader,
                        daemon=True
                    )
    ...
    def estimate_biases(self, request, response):
        ...
        response.success = True
        response.message = (
            f'Bias Estimate Set. x bias = {self.bias_body_x}, '
            f'y bias = {self.bias_body_y}, z bias = {self.bias_body_z}'
        )
        return response
