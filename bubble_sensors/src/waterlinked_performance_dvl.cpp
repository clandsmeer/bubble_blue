...
private:
    void odom_callback(const nav_msgs::msg::Odometry &msg)
    {
        ...
        inaccurate_linear_twist_.x = msg.twist.twist.linear.x * (long_term_inaccuracies_ * (double)std::rand() / RAND_MAX);
        inaccurate_linear_twist_.y = msg.twist.twist.linear.y * (long_term_inaccuracies_ * (double)std::rand() / RAND_MAX);
        inaccurate_linear_twist_.z = msg.twist.twist.linear.z * (long_term_inaccuracies_ * (double)std::rand() / RAND_MAX);

        // now add some noise
        noisy_msg.twist.twist.linear.x += std::round((inaccurate_linear_twist_.x + diff(rng_generator_)) / resolution_) * resolution_;
        noisy_msg.twist.twist.linear.y += std::round((inaccurate_linear_twist_.y + diff(rng_generator_)) / resolution_) * resolution_;
        noisy_msg.twist.twist.linear.z += std::round((inaccurate_linear_twist_.z + diff(rng_generator_)) / resolution_) * resolution_;
