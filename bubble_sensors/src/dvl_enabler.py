...
class DVLConfigurator(Node):
    def __init__(self):
        ...
        self.publisher_ = self.create_publisher(ConfigCommand, '/dvl/config/command', 10)
        # Wait 3 seconds before sending
        self.declare_parameter('timer_wait', 3.0)
        self.timer_wait = self.get_parameter('timer_wait').get_parameter_value().double_value
    ...
    def send_config(self):
        if not self.sent:
            ...
            self.get_logger().info(f'Sent acoustic_enabled = true after {self.timer_wait} seconds')
            self.sent = True
